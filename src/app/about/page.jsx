    ...
    <div className='overflow-hidden'>
      <div className='h-40 w-full mt-12 relative'>
        <Image src={smallBanner}
          className="absolute object-cover top-0 left-0 w-full h-full rounded-md" alt="banner Image" />
        <div className="absolute top-0 left-0 w-full h-full bg-black opacity-20"></div>
        <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 '>
          <motion.h1 className='text-sm md:text-3xl lg:text-5xl text-white font-bold'
          initial={{ y:20, opacity: 0 }}
           whileInView={{ y:0,opacity: 1 }}
            transition={{ duration:1, ease: 'easeIn',delay:.2 }}>We Make Your Home Safe</motion.h1>
        </div>
      </div>
      <div className="container py-10 px-5">
        ...
        transition={{ duration:1, ease: 'easeIn',delay:.2 }}>What We Do<span className='text-primary font-extrabold ml-3 '>&#x2192;</span></motion.h2>
           ...
           <motion.p className='text-justify px-5 py-3 text-gray-500 text-xl leading-10' initial={{ x:-60,opacity: 0 }}
        whileInView={{ x:0,opacity: 1 }}
